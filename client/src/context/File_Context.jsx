...
function FileContextProvider({ children }) {
    ...
    const renameDirectory = useCallback(
        (dirId, newDirName, sendToSocket = true) => {
            ...
            socket.emit("DIRECTORY_RENAMED", { // SocketEvent.DIRECTORY_RENAMED -> "DIRECTORY_RENAMED"
                dirId,
                newDirName,
            });
    ...
    const deleteDirectory = useCallback(
        (dirId, sendToSocket = true) => {
            ...
            if (!sendToSocket) return;
            socket.emit("DIRECTORY_DELETED", { dirId }); // SocketEvent.DIRECTORY_DELETED -> "DIRECTORY_DELETED"
        },
        [socket],
    );
    ...
    const createFile = useCallback(
        (parentDirId, file, sendToSocket = true) => {
            ...
            socket.emit("FILE_CREATED", { // SocketEvent.FILE_CREATED -> "FILE_CREATED"
                parentDirId,
                newFile,
            });
    ...
    const renameFile = useCallback(
        (fileId, newName, sendToSocket = true) => {
            ...
            if (!sendToSocket) return true;
            socket.emit("FILE_RENAMED", { // SocketEvent.FILE_RENAMED -> "FILE_RENAMED"
                fileId,
                newName,
            });
    ...
    const deleteFile = useCallback(
        (fileId, sendToSocket = true) => {
            ...
            if (!sendToSocket) return;
            socket.emit("FILE_DELETED", { fileId }); // SocketEvent.FILE_DELETED -> "FILE_DELETED"
        },
    ...
    const handleUserJoined = useCallback(
        ({ user }) => {
            ...
            socket.emit("SYNC_FILE_STRUCTURE", { // SocketEvent.SYNC_FILE_STRUCTURE -> "SYNC_FILE_STRUCTURE"
                fileStructure,
                openFiles,
                activeFile,
            ...
            socket.emit("SYNC_DRAWING", { // SocketEvent.SYNC_DRAWING -> "SYNC_DRAWING"
                drawingData,
                socketId: user.socketId,
            });
    ...
    useEffect(() => {
        socket.once("SYNC_FILE_STRUCTURE", handleFileStructureSync); // SocketEvent.SYNC_FILE_STRUCTURE -> "SYNC_FILE_STRUCTURE"
        socket.on("USER_JOINED", handleUserJoined); // SocketEvent.USER_JOINED -> "USER_JOINED"
        socket.on("DIRECTORY_CREATED", handleDirCreated); // SocketEvent.DIRECTORY_CREATED -> "DIRECTORY_CREATED"
        socket.on("DIRECTORY_UPDATED", handleDirUpdated); // SocketEvent.DIRECTORY_UPDATED -> "DIRECTORY_UPDATED"
        socket.on("DIRECTORY_RENAMED", handleDirRenamed); // SocketEvent.DIRECTORY_RENAMED -> "DIRECTORY_RENAMED"
        socket.on("DIRECTORY_DELETED", handleDirDeleted); // SocketEvent.DIRECTORY_DELETED -> "DIRECTORY_DELETED"
        socket.on("FILE_CREATED", handleFileCreated); // SocketEvent.FILE_CREATED -> "FILE_CREATED"
        socket.on("FILE_UPDATED", handleFileUpdated); // SocketEvent.FILE_UPDATED -> "FILE_UPDATED"
        socket.on("FILE_RENAMED", handleFileRenamed); // SocketEvent.FILE_RENAMED -> "FILE_RENAMED"
        socket.on("FILE_DELETED", handleFileDeleted); // SocketEvent.FILE_DELETED -> "FILE_DELETED"

        return () => {
            socket.off("USER_JOINED"); // SocketEvent.USER_JOINED -> "USER_JOINED"
            socket.off("DIRECTORY_CREATED"); // SocketEvent.DIRECTORY_CREATED -> "DIRECTORY_CREATED"
            socket.off("DIRECTORY_UPDATED"); // SocketEvent.DIRECTORY_UPDATED -> "DIRECTORY_UPDATED"
            socket.off("DIRECTORY_RENAMED"); // SocketEvent.DIRECTORY_RENAMED -> "DIRECTORY_RENAMED"
            socket.off("DIRECTORY_DELETED"); // SocketEvent.DIRECTORY_DELETED -> "DIRECTORY_DELETED"
            socket.off("FILE_CREATED"); // SocketEvent.FILE_CREATED -> "FILE_CREATED"
            socket.off("FILE_UPDATED"); // SocketEvent.FILE_UPDATED -> "FILE_UPDATED"
            socket.off("FILE_RENAMED"); // SocketEvent.FILE_RENAMED -> "FILE_RENAMED"
            socket.off("FILE_DELETED"); // SocketEvent.FILE_DELETED -> "FILE_DELETED"
        };
    }, [
        handleDirCreated,
        handleDirDeleted,
