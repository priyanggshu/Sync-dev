...
.animated-gradient {
  ...
  animation: rotateGradient 8s ease infinite;
      ...
      <div className="hidden md:flex md:w-1/2 md:bg-gradient-to-br md:from-[#edede9] md:to-transparent flex-col justify-center items-center p-10 text-center bg-opacity-30 rounded-2xl shadow-lg">
        ...
        <img src={auth} alt="Signup Illustration" className="max-w-sm mt-6 rounded-xl shadow-md" />
      </div>
      ...
      <div className="relative md:bg-gradient-to-br md:from-[#e8e7e2] md:to-blue-100 md:border border-gray-700 p-10 rounded-xl shadow-2xl max-w-md w-full md:w-1/2 flex flex-col items-center">
        <img src={auth} alt="Signup Illustration" className="absolute inset-0 w-full h-full object-cover opacity-55 rounded-xl md:hidden" />
        <h2 className="text-4xl font-bold text-center mb-8 text-gray-900 relative z-10">Create Your Account</h2>
        <form onSubmit={handleSubmit} className="space-y-4 w-full relative z-10">
          <input type="text" name="username" placeholder="👤 Username" value={form.username} onChange={handleChange} className={`${inputStyle} shadow-xl placeholder:text-white`} />
          ...
          <button type="submit" className={`${btnPrimary} mt-6`}>Sign Up ✨</button>
        </form>
        <button onClick={handleGoogleSignup} className="mt-6 py-3 flex items-center justify-center w-full font-semibold bg-[#FDC987] text-white md:text-gray-500 border p-3 rounded-lg hover:bg-indigo-200 shadow-sm transition relative gap-1 z-10">
          <FcGoogle className="mr-2 scale-150 text-2xl" /> Sign up with Google
        </button>
        <p className="text-center font-light mt-4 text-gray-900 md:text-gray-600 relative z-10">Already have an account? <Link to="/login" className="text-blue-600 font-semibold">Login 🔥</Link></p>
      </div>
    </div>
  );
}
