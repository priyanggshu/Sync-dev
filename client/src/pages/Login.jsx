  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center gap-10 px-8 py-6 md:flex-row md:py-0 md:px-24 bg-gray-50">
      ...
      <div className="md:w-3/5 hidden md:flex flex-col justify-center items-center text-center p-16 bg-gradient-to-br from-blue-100 to-blue-300 rounded-lg shadow-lg">
        ...
        <img src={auth} alt="Login Illustration" className="w-xl mt-4 rounded-lg shadow-md" />
      </div>

      {/* Right Section - Login Form */}
      <div className="w-full md:w-3/5 lg:w-2/5 bg-white p-10 md:p-14 rounded-xl shadow-lg flex flex-col items-center">
        <h2 className="text-4xl font-bold text-center mb-6 text-gray-900">Welcome Back</h2>
        ...
        <button onClick={handleGoogleLogin} className="mt-6 py-3 flex items-center justify-center w-full font-semibold bg-gray-200 text-gray-700 border p-3 rounded-lg hover:bg-blue-100 shadow-sm transition">
          <FcGoogle className="mr-2 text-2xl" /> Login with Google
        </button>
        <p className="text-center font-light mt-4 text-gray-600">Don't have an account? <Link to="/signup" className="text-blue-600 font-semibold">Sign Up ✨</Link></p>
      </div>
    </div>
  );
}
